...
import pool.MongoConnection;

/**
 * Database implementation of the ModelDAO interface. This class provides the
 * concrete implementation for all data access operations including user
 * registration, authentication, profile management, and administrative
 * functions. It handles database connections, SQL execution, transaction
 * management, and error handling for the entire application data layer.
 *
...
public class DBImplementation implements ModelDAO {
    ...
    private final String collectionDb = "perfiles";
    // Instancia única de la base de datos (Singleton)
    private final MongoDatabase db = MongoConnection.getDatabase();
    // Referencia directa a la colección "perfiles"
    private final MongoCollection<Document> collection = db.getCollection("perfiles");
    /**
     * SQL Queries: INSERTS
     */
    ...
    /**
     * Inserts a new user into the database with transaction support. This
     * method performs an atomic operation that inserts user data into both the
     * profile and user tables within a single transaction. If any part fails,
    ...
    /**
     * Retrieves all users from the database. This method executes a query to
     * fetch all user records with their complete profile information including
     * personal details and preferences.
     *
     * @param con the database connection to use for the operation
     * @return an ArrayList containing all User objects from the database
     * @throws OurException if the query execution fails or data retrieval
     * errors occur
     */
    ...
    /**
     * Updates an existing user's information in the database with transaction
     * support. This method performs an atomic operation that updates user data
     * in both the profile and user tables within a single transaction.
     *
     * @param con the database connection to use for the operation
    ...
    /**
     * Deletes a user from the database by their unique identifier. This method
     * removes a user record from the system based on the provided user ID.
     *
     * @param con the database connection to use for the operation
     * @param userId the unique identifier of the user to be deleted
     * @return true if the deletion was successful, false if no user was found
     * with the specified ID
     * @throws OurException if the deletion operation fails due to SQL errors or
    ...
    /**
     * Authenticates a user by verifying credentials against the database. This
     * method checks if the provided credential (email or username) and password
     ...
     * @return the authenticated user's Profile object (User or Admin) if
     * credentials are valid, null otherwise
     * @throws OurException if the authentication process fails due to SQL
     * errors or data retrieval issues
     */
     ...
     * This method verifies the uniqueness of user credentials during
    ...
    /**
     * Waits for a database connection to become available with timeout
     * protection. This method implements a polling mechanism to wait for a
     * connection thread to become ready, preventing indefinite blocking.
     *
    ...
    /**
     * Rolls back the current database transaction. This method provides safe
     * transaction rollback with proper error handling for scenarios where
    ...
    /**
     * Resets the auto-commit mode of the database connection to true. This
     * method ensures that the connection returns to its default auto-commit
    ...
    /**
     * Authenticates a user with the provided credentials. This method verifies
     * user identity by checking the provided credential and password against
     * stored user data and sets the logged-in profile upon successful
     * authentication.
     *
     * @param credential the user's username or email address used for
     * identification
     * @param password the user's password for authentication
     * @return the authenticated user's Profile object containing user
     * information and access privileges, or null if authentication fails
     * @throws OurException if authentication fails due to database errors or
     * system issues
     */
    @Override
    public Profile login(String credential, String password) throws OurException {
        try {
            // Buscamos coincidencia exacta con P_USERNAME y P_PASSWORD
            Document doc = collection.find(and(eq("P_USERNAME", credential), eq("P_PASSWORD", password))).first();

            if (doc != null) {
                // Aquí ocurre la magia: convierte JSON -> Objeto Java
                Profile profile = mapDocumentToProfile(doc);

                // Guardamos la sesión en tu singleton
            ...
            // Esto pasa si en el JSON hay un género que no existe en tu Enum (ej: "ALIEN")
            System.err.println("Error en el Enum de Género: " + e.getMessage());
            throw new OurException(ErrorMessages.LOGIN);
        } catch (Exception ex) {
            ex.printStackTrace();
            throw new OurException(ErrorMessages.LOGIN);
     ...
     * @return the registered User object with the generated ID and
    ...
    /**
     * Retrieves a list of all users from the system. This method provides
     * access to the complete user database, typically used by administrative
     * interfaces for user management operations.
     *
    ...
    /**
     * Updates an existing user's information in the system. This method
     * persists changes made to a user's profile data, ensuring that
     ...
     * @throws OurException if the update operation fails due to validation
     * errors, database constraints violations, or data access issues
     */
    @Override
    ...
    /**
     * Deletes a user from the system by their unique identifier. This method
     * permanently removes a user record from the database based on the provided
     * user ID.
     *
    ...
    private Profile mapDocumentToProfile(Document doc) {
        // 1. Extraer datos COMUNES (presentes en todos los perfiles)
        // Usamos las claves EXACTAS de tu JSON (con mayúsculas P_...)
        int id = doc.getInteger("_id");
        String email = doc.getString("P_EMAIL");
        String username = doc.getString("P_USERNAME");
        String password = doc.getString("P_PASSWORD");
        String name = doc.getString("P_NAME");
        String lastname = doc.getString("P_LASTNAME");
        String phone = doc.getString("P_TELEPHONE");

        // 2. Detectar si es ADMIN
        // Miramos si existe el sub-documento "admin"
        if (doc.containsKey("admin") && doc.get("admin") != null) {

            Document adminData = (Document) doc.get("admin");
            String currentAccount = adminData.getString("A_CURRENT_ACCOUNT");

            // Usamos tu constructor de Admin
            return new Admin(id, email, username, password, name, lastname, phone, currentAccount);
        } // 3. Detectar si es USER
        // Miramos si existe el sub-documento "user"
        else if (doc.containsKey("user") && doc.get("user") != null) {

            Document userData = (Document) doc.get("user");
            String card = userData.getString("U_CARD");

            // Conversión de String a Enum Gender
            String genderStr = userData.getString("U_GENDER");
            Gender gender = Gender.valueOf(genderStr); // Convierte "MALE" -> Gender.MALE

            // Usamos tu constructor de User
            return new User(id, email, username, password, name, lastname, phone, gender, card);
        }

        return null; // Si el documento no tiene formato válido
    }
}
